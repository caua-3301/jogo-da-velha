/*Desenvolvido por: Cauã Pires Soares - graduando em análise e desenvolvimento de sistemas
Data:25/08/2023

O jogo foi desenvolvido com o objetivo de expandir meus conhecimentos com relaçao a linguagem, e também,
melhorar minha lógica e capacidade de resolver problemas
*/
...
//Função para auxiliar na criação da estrutura do jogo
...
//Criando a estrutura do jogo, com a linhas verticais e horizontais (i = linha, j = coluna, isso em todo o código)
void criar_jogo(char matriz[ORDEM][ORDEM]){
	...
	//Invés de deixar os indices para jogar (1, 3, 5), resolvi deixar 1, 2 e 3, para uma melhor experiencia do usuario (todavia, gera mais codigo)
...
//Procedimento responsável por printar o jogo
...
//Ajustando indice de jogadas (nesse caso, o 1 não é alterado, o 2 altera para 3, e o 3 para 5, (esses são os verdadeiros indices da tabela)) 
int ajuste(int eixo){
	if(eixo != 1){
...
//Função responsável por adicionar X ou O no jogo
...
void jogada_computador(char matriz[ORDEM][ORDEM], char simbolo){
	...
	//O computador jogará apenas quando encontrar um espaço livre na matriz (e dentro dos blocos permitidos)
	while (jogada_comp == 0){
		if (matriz[eixo_x][eixo_y] == ' '){
			matriz[eixo_x][eixo_y] = simbolo;
			jogada_comp = 1;
...
void main(){
	...
	printf ("INSTRUÇÕES: Para jogar, voce deve informar dois numeros, primeiro, o da linha (valores a esqueda do jogo da velha)\n");
	...
	while (opcao == 1 || opcao == 2 && opcao != 3){
		...
		if (opcao == 1){
			...
			for (numero_jogadas; numero_jogadas<JOGADAS; numero_jogadas++){
				...
				printf ("\n\nVez do jogador %d\n", jogador);
				scanf ("%d%d", &x, &y);
				//Chegando qual jogador será o próximo, bem como o símbulo (X ou O)
		...
		if (opcao == 2){
			...
			for (numero_jogadas; numero_jogadas<JOGADAS; numero_jogadas++){
				...
				//Solicitando as coordenadas para o usuário
				if (jogador == 1){
					printf ("\n\nVez do jogador %d\n", jogador);
					scanf ("%d%d", &x, &y);
					jogada(velha, x, y, simbolo);
				...
				//Checando qual jogador será o próximo, bem como o símbulo (X ou O)
				jogador = jogador == 1 ? 2 : 1;
				simbolo = simbolo == 'X' ? 'O' : 'X';
			}
		}
